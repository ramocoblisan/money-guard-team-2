...
export function Form({
  ...
  useEffect(() => {
    if (content) {
      ...
      setValue('amount', +content.amount.toString().replace('-', ''));
      setValue('comment', content.comment);
      setStartDate(new Date(content.transactionDate)); // Setează data inițială
    } else {
      setType('EXPENSE');
    }
  }, [content, setValue]);
  ...
  const submit = data => {
    ...
    if (typeForm === 'edit') {
      updatedData.transactionDate = formatDate(startDate); // Include data actualizată
    }
          ...
          <div className={style.boxDate}>
            ...
            <div className={style.svgBox}>
              <DatePicker
                ...
                wrapperClassName={style.reactDatepickerWrapper}
                calendarClassName={style.reactDatepickerCalendar}
              />
